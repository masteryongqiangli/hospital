...
public class DataDictionaryDaoImpl extends BaseDaoImpl implements
	...
	public <T> List<Sys_Base_DataDictionary> getParentDataDicList() {
		return super.createCriteria(Sys_Base_DataDictionary.class)
				.add(Restrictions.isNull("parent_DataDictionary"))
				.add(Restrictions.eq("state", 1)).list();
	}

	@SuppressWarnings("unchecked")
	public <T> List<Sys_Base_DataDictionary> getAllDataDicList(
		...
		if (StringUtil.isNotEmpty(parms.get("parent_id"))) {
			...
			criteria.add(Restrictions.or(
					Restrictions.eq("parent_DataDictionary",
							parms.get("parent_id")),
					Restrictions.eq("dataDicId", parms.get("parent_id"))));

		}
		return criteria.add(Restrictions.eq("state", 1))
				.addOrder(Order.asc("parent_DataDictionary"))
	...
	public int batchDelete(Sys_Base_DataDictionary sys_Base_DataDictionary) {
		...
		query.setParameter("parent_DataDictionary",
				sys_Base_DataDictionary.getDataDicId());
	...
	public Map<String, List<Sys_Base_DataDictionary>> getSelects(String[] codes,String flag) {
		...
		List<Map<String, Object>> datas = query.setResultTransformer(
