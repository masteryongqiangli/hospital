...
public class MenuDaoImpl extends BaseDaoImpl implements MenuDaoI {
	...
	public <T> List<Sys_Base_Menu> getUserMenuList() {
		...
		sql.append(ResourceUtil.getSys_User().getUserId()
				+ "' order by a.parent_menu_menuId,a.orderNum");
	...
	public <T> List<Sys_Base_Menu> getAllMenuList(Map<String, String> parms) {
		...
		criteria.addOrder(Order.asc("parent_menu")).addOrder(
				Order.asc("orderNum"));
	...
	public <T> List<Sys_Base_Menu> getParentMenuList() {
		return super.createCriteria(Sys_Base_Menu.class)
				.add(Restrictions.isNull("parent_menu"))
